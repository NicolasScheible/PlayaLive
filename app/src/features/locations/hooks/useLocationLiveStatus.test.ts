...
describe('useLocationLiveStatus', () => {
  ...
  it('lädt den Live-Status der Location', async () => {
    ...
    const { result } = renderHook(() => useLocationLiveStatus('loc-1'), {
  ...
  it('abonniert Report-Updates, solange der Screen fokussiert ist', async () => {
    ...
    renderHook(() => useLocationLiveStatus('loc-1'), { wrapper: createQueryWrapper() });
  ...
  it('beendet die Subscription beim Unmount', async () => {
    ...
    const { unmount } = renderHook(() => useLocationLiveStatus('loc-1'), {
      wrapper: createQueryWrapper(),
    });
